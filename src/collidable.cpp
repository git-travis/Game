...
Collidable::Collidable(std::string name){
	...
	if(_texture.loadFromFile(_filename)){
		...
		std::cout << "Could not load file: collidable" << std::endl;
	}
}
...
void Collidable::build(){
	if(_built){
		for(int i = 0; i < _sizeX; i++){
			_linksX[i].setPosition(_pos.x + (_texture.getSize().x * i) - (_texture.getSize().y / 2), _pos.y);
			_linksX[i + _sizeX].setPosition(_pos.x + (_texture.getSize().x * i) - (_texture.getSize().y / 2), _pos.y + (_texture.getSize().x * _sizeY));
		}
		for(int i = 0; i < _sizeY; i++){
			_linksY[i].setPosition(_pos.x, _pos.y + (_texture.getSize().x * i) + (_texture.getSize().y / 2));
			_linksY[i + _sizeY].setPosition(_pos.x + (_texture.getSize().x * _sizeX), _pos.y + (_texture.getSize().x * i) + (_texture.getSize().y / 2));
		}
	}
	else{
		for(int i = 0; i < _sizeX; i++){
		...
		for(int i = _sizeX; i < _sizeX + _sizeX; i++){
			_linksX.push_back(sf::Sprite(_texture));
			_linksX[i].setPosition(_pos.x + (_texture.getSize().x * (i - _sizeX))  - (_texture.getSize().y / 2), _pos.y + (_texture.getSize().x * _sizeY));
		}
		for(int i = 0; i < _sizeY; i++){
		...
		for(int i = _sizeY; i < _sizeY + _sizeY; i++){
			_linksY.push_back(sf::Sprite(_texture));
			_linksY[i].setRotation(90);
			_linksY[i].setPosition(_pos.x + (_texture.getSize().x * _sizeX) , _pos.y +  + (_texture.getSize().x * (i - _sizeY)) + (_texture.getSize().y / 2));
		}
		_built = true;
	}
}
