...
Options::Options(std::string textureName):
...
void Options::initText(){
	...
	text.setString("Graphics Settings");
...
menuStatus Options::update(sf::Vector2f mousePos, sf::Vector2f* view, Game* game, sf::RenderWindow* window, bool mouseClick, menuStatus prev, sf::Event event){
	if(_counter < 1){
		_back = prev;
		_counter++;
	}
	...
	if(mouseClick){
		for(unsigned int i = 0; i < _sprites.size(); i++){
			...
			if((game->checkXColl(_texture.getSize().x / 4, mousePos, _sprites.at(i).getPosition())) && (game->checkYColl(_texture.getSize().y / 4, mousePos, _sprites.at(i).getPosition()))){
				if(_text.at(i).getString() == "Graphics Settings"){
					return GRAPHICS;
				}
				if(_text.at(i).getString() == "Audio"){
					return AUDIO;
				}
